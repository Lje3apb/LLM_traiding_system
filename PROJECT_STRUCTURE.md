...
1. **Агрессивная оценка позиций** (`position_sizing.py`):
   - ✅ Полная переработка алгоритма с нелинейной амплификацией
   - ✅ Новые гиперпараметры: EDGE_GAIN=2.5, EDGE_GAMMA=0.7, BASE_K=0.5
   - ✅ Confidence-based scaling: low (0.7x), medium (1.0x), high (1.3x)
   - ✅ Trend strength alignment: 0.8-1.2x динамический бустер
   - ✅ Нейтральный режим теперь k=0.5 (вместо k=1.0)

2. **Смягченная LLM калибровка** (`market_snapshot.py`):
   - ✅ Системный промпт обновлен для менее консервативных вероятностей
   - ✅ Разрешены вероятности 0.6-0.85 при согласованных сигналах
   - ✅ Few-shot примеры для лучшей калибровки LLM

3. **Обновлены тесты** (`tests/`):
   - ✅ Все 19 тестов в test_position_sizing.py переписаны под новую логику
   - ✅ Новый standalone скрипт test_full_cycle.py с argparse
   - ✅ Автоматическая настройка Python path (работает из любой директории)
...
5. **UI баги исправлены**:
   ...
   - ✅ Static file paths (url_for usage)
...
├── test_full_cycle.py               # Standalone скрипт полного интеграционного теста
...
- **Логика (АГРЕССИВНЫЙ РЕЖИМ)**:
  - **Гиперпараметры**:
    - `EDGE_GAIN = 2.5` - усиление края (edge amplification)
    - `EDGE_GAMMA = 0.7` - нелинейное сжатие (power function)
    - `BASE_K = 0.5` - базовый множитель для нейтрального режима
  - **Расчет edge**: `edge = prob_bull - 0.5`
  - **Нелинейное усиление**: `edge_eff = (|edge| * EDGE_GAIN) ^ EDGE_GAMMA`
  - **Confidence scaling**: low: 0.7x, medium: 1.0x, high: 1.3x
  - **Trend alignment**: 0.8 + 0.4 * trend_strength (бустер 0.8-1.2x)
  - **Risk throttling**: 30% штраф за комбинированный риск (ликвидность + новости)
  - **Множители**:
    - Bullish: `k_long = BASE_K + edge_eff`, `k_short = BASE_K - edge_eff`
    - Bearish: `k_long = BASE_K - edge_eff`, `k_short = BASE_K + edge_eff`
- **Функции**:
  - Агрессивная нелинейная амплификация сигнала
  - Адаптация к уверенности LLM (confidence_level)
  - Усиление по силе тренда (trend_strength)
...
- **Ключевые функции**:
  ...
  - `build_system_prompt()`: Генерация системных инструкций для LLM (ОБНОВЛЕНО)
...
- **Системный промпт (АГРЕССИВНЫЙ РЕЖИМ)**:
  - Смягченная калибровка вероятностей: 0.60–0.75 приемлемо при согласованных сигналах
  - Разрешены вероятности 0.75–0.85 при сильных доказательствах
  - Few-shot примеры калибровки для LLM
  - Избегание сверх-консервативности (не всегда 0.5-0.55)
...
- ~19 тестовых случаев (все обновлены)
- **Изменения**:
  - Нейтральный режим: ожидается k=0.5 (вместо k=1.0)
  - Тесты confidence_level (вместо btc_sentiment)
  - Тесты trend_strength (вместо onchain_pressure)
  - Проверка относительного поведения (k_long > k_short)

#### **test_full_cycle.py**
- **Standalone интеграционный скрипт** (НЕ pytest тест!)
- **Расположение**: корень проекта + tests/test_full_cycle.py
- **Использование**: `python test_full_cycle.py [OPTIONS]`
- **Опции**:
  - `--real-data`: Использовать реальные API данные
  - `--model MODEL`: Выбор модели Ollama (по умолчанию: llama3.2)
  - `--ollama-url URL`: URL Ollama API
- **Функции**:
  - Автоматическая настройка Python path (работает из любой директории)
  - Генерация mock данных для быстрого тестирования
...
- Руководство по сквозному тестированию (ОБНОВЛЕНО)
- Описание агрессивного режима position sizing
- Параметры: EDGE_GAIN=2.5, EDGE_GAMMA=0.7, BASE_K=0.5
- Ожидаемые выводы и интерпретация результатов
- Инструкции по тюнингу агрессивности
- Устранение неполадок
- Использование standalone скрипта test_full_cycle.py
...
# Полный цикл теста (CLI)
python -m llm_trading_system.cli.full_cycle_cli

# Полный цикл теста (standalone скрипт) - НОВЫЙ СПОСОБ
python test_full_cycle.py --real-data --model deepseek-v3.1:671b-cloud
python tests/test_full_cycle.py  # Также работает из tests/
