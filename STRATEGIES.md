...
0. [Загрузка исторических данных](#0-загрузка-исторических-данных)
...
## 0. Загрузка исторических данных

Для запуска бэктестов нужны исторические данные OHLCV (Open, High, Low, Close, Volume). Система поддерживает автоматическую загрузку данных с архива Binance.

### 0.1 Автоматическая загрузка через Web UI

**Шаг 1**: Откройте страницу бэктеста для вашей стратегии

Перейдите на страницу `http://127.0.0.1:8000/ui/strategies/ваша_стратегия/backtest`

**Шаг 2**: Заполните параметры загрузки данных

В секции **"Data Download from Binance"** вы увидите:

- **Symbol**: Торговая пара (например, `BTCUSDT`, `ETHUSDT`)
  - Автоматически заполняется из конфига стратегии

- **Interval**: Интервал свечей
  - `1m` — 1 минута
  - `5m` — 5 минут (рекомендуется для начала)
  - `15m` — 15 минут
  - `30m` — 30 минут
  - `1h` — 1 час
  - `4h` — 4 часа
...
- **Start Date** и **End Date**: Период данных
  - Формат: `YYYY-MM-DD` (например, `2024-01-01`)
  - Рекомендуется начинать с небольшого периода (1-3 месяца)

**Шаг 3**: Нажмите кнопку **"Download Data"**

Система автоматически:
1. Скачает данные с `data.binance.vision` (официальный архив Binance)
2. Обработает и проверит корректность временных меток
3. Сохранит в CSV файл в папке `data/`
4. Автоматически подставит путь к файлу в поле **"Data File Path"**

**Шаг 4**: Запустите бэктест

После успешной загрузки:
- Поле "Data File Path" будет автоматически заполнено
- Проверьте остальные параметры (Initial Equity, Fee Rate)
- Нажмите **"Run Backtest"**

### 0.2 Формат CSV файлов

Скачанные данные сохраняются в стандартном формате:

```csv
timestamp,open,high,low,close,volume
2024-01-01 00:00:00,42250.5,42380.2,42100.0,42300.1,1250.5
2024-01-01 00:05:00,42300.1,42450.0,42280.0,42420.5,980.3
...
```

**Формат имени файла**: `{symbol}_{interval}_{start_date}_{end_date}.csv`

Примеры:
- `BTCUSDT_5m_2024-01-01_2024-03-31.csv`
- `ETHUSDT_1h_2024-06-01_2024-12-31.csv`

### 0.3 Кэширование данных

Система автоматически кэширует скачанные данные:

- При повторной загрузке **того же периода** данные берутся из кэша (папка `data/`)
- Не нужно повторно скачивать данные для разных стратегий на одном символе
- Для принудительной перезагрузки удалите соответствующий CSV файл

### 0.4 Программная загрузка (опционально)

Если нужно загрузить данные программно:

```python
from llm_trading_system.data import get_data_manager

# Получить менеджер данных
dm = get_data_manager()

# Загрузить данные
filepath, row_count = dm.get_or_download_data(
    symbol="BTCUSDT",
    interval="5m",
...
print(f"Downloaded {row_count} rows to {filepath}")
```

### 0.5 Возможные ошибки и решения

**Ошибка**: "Data not found for {date}"
- **Причина**: Данные для указанной даты отсутствуют на Binance
- **Решение**: Проверьте правильность символа и даты (Binance spot начал работу в 2017 году)

**Ошибка**: "Network error"
- **Причина**: Проблемы с интернет-соединением
- **Решение**: Проверьте подключение к интернету

**Предупреждение**: "Large date range (X days) may take a while"
- **Причина**: Запрашивается период > 1 года
- **Решение**: Подождите или разбейте на меньшие периоды

### 0.6 Поддерживаемые источники данных

**Текущая версия**:
- ✅ Binance Spot (data.binance.vision)
- ✅ Все стандартные интервалы (1m, 5m, 15m, 30m, 1h, 4h, 1d)
