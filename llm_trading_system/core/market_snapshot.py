...
def build_system_prompt() -> str:
    ...
    return (
        ...
        "4. UNCERTAINTY HANDLING AND PROBABILITY CALIBRATION:\n"
        "   - If the provided data is sparse, low-impact, or contradictory, keep probabilities closer to 0.5\n"
        "     and set confidence_level = \"low\".\n"
        "   - You are a probabilistic estimator, but NOT overly conservative:\n"
        "     * 0.50–0.60: weak bias or unclear signals\n"
        "     * 0.60–0.75: moderate bias with aligned signals (ACCEPTABLE when signals point in same direction)\n"
        "     * 0.75–0.85: strong bias with consistent evidence (ACCEPTABLE when input strongly supports)\n"
        "     * >0.85: very strong bias (only use when evidence is overwhelmingly one-sided)\n"
        "   - Avoid ultra-extreme values above 0.95 or below 0.05 unless evidence is extremely one-sided.\n"
        "     Otherwise, more confident probabilities (0.6–0.85) are acceptable and encouraged when signals\n"
        "     consistently point in one direction.\n\n"
        "   EXAMPLE CALIBRATION (for reference, not executable):\n"
        "   * Bullish scenario: Strong positive price trend (+5% 24h), positive news sentiment (0.6),\n"
        "     on-chain inflows, rising open interest, positive funding rate.\n"
        "     → Appropriate output: prob_bull ≈ 0.75, prob_bear ≈ 0.25, regime_label = \"bull\",\n"
        "     confidence = \"medium\" or \"high\".\n\n"
        "   * Bearish scenario: Negative price trend (-6% 24h), negative news sentiment (-0.5),\n"
        "     on-chain outflows, declining addresses, rising liquidity risk.\n"
        "     → Appropriate output: prob_bull ≈ 0.25, prob_bear ≈ 0.75, regime_label = \"bear\",\n"
        "     confidence = \"medium\" or \"high\".\n\n"
        "5. OUTPUT FORMAT:\n"
